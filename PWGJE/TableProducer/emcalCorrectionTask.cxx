...
struct EmcalCorrectionTask {
  ...
  void init(InitContext const&)
  {
    ...
    hCollisionMatching.setObject(new TH1F("hCollisionMatching", "hCollisionMatching", 3, -0.5, 2.5)); // 0, no vertex
  }
  ...
  void process(aod::BC const& bc, aod::Collisions const& collisions, aod::Tracks const& tracks, aod::Calos const& cells)
  {
    ...
    for (auto& clusterizer : mClusterizers) {
      ...
      if (collisions.size() > 1) {
        if (i == 0)
          hCollisionMatching->Fill(2); // found more than one collision for BC
        ...
        for (const auto& col : collisions) {
          ...
          vz = col.posZ();
          if (i == 0)
            hCollisionMatching->Fill(1); // found one collision for BC (only fill for first clusterizer)
          hasCollision = true;
      ...
      if (!hasCollision) {
        if (i == 0)
          hCollisionMatching->Fill(0); // found no collision for BC
